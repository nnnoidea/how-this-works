...
Reuse architecture studies, Agent indexes, and versioned evidence. The Agent judges classification, mechanisms and the meaning of changes. Scripts handle identity, deduplication, bounded selection, retained versions, work lists, and rendering. A project can have several capabilities; a primary classification prevents duplicate counts within an aggregate.

## Choose each collection request

Select the ranking basis, scope, period, and N for the current request. Do not assume cumulative stars, monthly growth, or Top 50. Candidates must retain their source, observation time and ordering basis. The script accepts an already ranked candidate list; it is not a universal ranking-data collector. Retrieve candidates through the API or collection tool appropriate to the chosen source, rather than inventing ranks.
