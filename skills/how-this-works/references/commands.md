...
The `--timezone` option of `render` sets a common display timezone. Monthly statistical indexes use UTC; account for that distinction when comparing months. See [Research judgments](research-contract.md) for time and historical interpretation boundaries.
