...
After initialization, submit semantic objects directly with `edit`; do not first generate request files or patches. Input fields and local editing rules are maintained only in the [Input contract](current-model.en.md).
...
Select the delivery target with `init --target architecture` or `--target complete`; the default is `complete`. Both start in the architecture stage. Old studies and imported models do not receive invented stage-completion records. Switching targets preserves all units and material relationships in the same study:
...
Allow only one write operation at a time per study directory. If a write lock is reported, first check for an active write. See `node "$skill_dir/scripts/materials/study.mjs" --help` for more arguments. The Agent need not recreate internal storage or locking logic.
